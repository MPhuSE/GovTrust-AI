...
└───┬───────────────────────────┬──────────────────────────┘
    ...
    ▼                          ▼
┌─────────────┐   ┌─────────────┐   ┌──────────────┐   ┌──────────────┐
│ MongoDB     │   │ ChromaDB/   │   │ Redis        │   │ Object Storage│
...
| Database | MongoDB (nghiệp vụ), ChromaDB/Qdrant (vector), Redis (queue/cache) |
...
### 5.2. Vector DB (FastAPI) — `legal_sources` (Chroma) / `legal_chunks` (Qdrant)
| Trường | Kiểu | Mô tả |
|---|---|---|
...
> ⚠️ **Mâu thuẫn cần chốt:** engine (ChromaDB vs Qdrant), tên collection (`legal_sources` vs `legal_chunks`), field TTL (`expiresAt` vs `expiredAt`), TTL (24h vs 30 phút), điểm trừ thiếu giấy tờ (-15 vs -20). Xem mục 9.
...
## 9. Vấn đề mở cần quyết định (Open Issues)
| # | Vấn đề | Phương án |
|---|---|---|
| OI-1 | Vector DB: ChromaDB hay Qdrant? | Chốt 1 engine + thống nhất tên collection |
| OI-2 | TTL session: 24h hay 30 phút? Field `expiresAt` hay `expiredAt`? | Chốt 1 giá trị + 1 tên field |
| OI-3 | Mô hình `sessions.aiResult` gộp vs tách nhiều collection | Chốt theo DATABASE_DESIGN (gộp) hoặc database.md (tách) |
| OI-4 | Điểm trừ thiếu giấy tờ: -15 hay -20? | Chốt trong `procedures.scoringRules` |
| OI-5 | Chính sách mock OCR: cấm hay cho phép fallback | Theo yêu cầu BTC |
