...
export class ProceduresService implements OnModuleInit {
  ...
  async findById(id: string): Promise<ProcedureDocument> {
    const p = await this.procedureModel.findById(id);
    if (!p) throw new NotFoundException(`Thủ tục với ID "${id}" không tồn tại`);
    return p;
  }
